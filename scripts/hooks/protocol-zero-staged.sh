#!/usr/bin/env bash
# protocol-zero-staged.sh
# Pre-commit wrapper -- runs Protocol Zero scan on staged files only.
# Delegates to scripts/protocol-zero.sh with a temp directory of staged content.
#
# Exit codes:
#   0  Clean
...
if [[ ! -x "$SCRIPT" ]]; then
    printf 'protocol-zero-staged: cannot find %s\n' "$SCRIPT" >&2
    exit 1
fi
